...
ContaPoupanca &operator>>( ContaPoupanca &c, double sub) {
...
void ContaPoupanca::extrato() const {
    ...
    cout << "Aniversário da conta:" << left << setfill('.') << setw(2) << " " << right << setfill('0') << setw(2) << getBday() << "/" 
    << setfill('0') << setw(2) << getBmonth() << "/" << getByear() << endl;
}

void ContaPoupanca::callWithdraw(double v) const {
    if (getBalance() - v < 0)
